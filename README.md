...
- 文章列表浏览与切换，支持按时间（正序/倒序）和标题（正序/倒序）排序
- 创建新文章（指定标题、分类、标签）
- 删除文章（移入回收站，可恢复）
...
- Markdown 编辑与实时预览联动（WebEngine 渲染），支持源码/预览模式切换
- 可调预览参数 — 正文字号、行间距、预览刷新延迟
- 自动保存（防抖机制）及 `Ctrl+S` 手动保存
- Front Matter 增强编辑 — title / date / categories / tags / cover / description 可视化编辑
- 封面图选择与导入（本地文件选择器）
- 文章内图片导入（自动复制到项目资源目录）

### AI 助手

- 缺失 description 时自动调用 AI（DeepSeek / GLM / OpenAI 兼容接口）生成摘要
- AI 对话面板 — 多轮对话式编辑，支持引用其他文章作为上下文
- AI 编辑模式 — 对话后生成修改建议，逐条 Diff 审查（接受/拒绝每处变更）
- 多 AI 提供者切换（none / deepseek / glm / openai），支持自定义 API Base、Model、API Key

### Hexo 命令集成

- 一键执行 `hexo generate`、`hexo deploy`、`hexo clean`、`hexo server`
- 内置控制台 — 可直接输入自定义命令，实时查看输出日志
- 环境检查 — 检测 Node.js、Hexo CLI、Git 是否可用
- 非 Hexo 目录自动识别并支持一键初始化项目
...
- Material Design 3 自定义主题（蓝色主色调）
- 自定义无边框窗口 — 支持 Aero Snap、边缘吸附、最大化/还原切换
- 可调节侧边栏文章列表与编辑区的布局
- 内置控制台面板（深色主题，可展开/收起）
- 右侧抽屉式配置中心（设置、项目管理、AI 配置、站点配置、回收站）
- 首次运行引导对话框
- 窗口大小变化时自动降级渲染以保持流畅
